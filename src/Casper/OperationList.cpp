...
Casper::OperationList::OperationList(const std::string& str) {
    branch = str;
}

void Casper::OperationList::addOperation(const Operation& operation) {
    operation_list.push_back(operation);
}

// Forge the given branch to a hex encoded string.
Data Casper::OperationList::forgeBranch() const {
...
Data Casper::OperationList::forge(const PrivateKey& privateKey) const {
